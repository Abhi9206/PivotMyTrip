...
export default function PlanningForm() {
  ...
  setFormData(prev => ({
    ...prev,
    destination: command.params.destination ?? prev.destination,
    days:
      command.params.days != null
        ? Math.max(1, Math.min(14, command.params.days))
        : prev.days,
    pace: command.params.pace ?? prev.pace,
    interests:
  command.params.interests && command.params.interests.length > 0
    ? Array.from(
        new Set(
          command.params.interests
            .map((interest) => interest.toLowerCase().trim())
            .map((interest) => {
              if (interest.includes('history')) return 'history';
              if (interest.includes('culture')) return 'history';
              if (interest.includes('food')) return 'food';
              if (interest.includes('dining')) return 'food';
              if (interest.includes('nature')) return 'nature';
              if (interest.includes('park')) return 'nature';
              if (interest.includes('shopping')) return 'shopping';
              if (interest.includes('shop')) return 'shopping';
              if (interest.includes('art')) return 'art';
              if (interest.includes('museum')) return 'art';
              if (interest.includes('nightlife')) return 'nightlife';
              if (interest.includes('night')) return 'nightlife';
              if (interest.includes('adventure')) return 'adventure';
              if (interest.includes('architecture')) return 'architecture';
              return null;
            })
    ...
    : prev.interests,
    start_location: command.params.start_location ?? prev.start_location,
    start_time: command.params.start_time ?? prev.start_time,
    transport_mode: command.params.transport_mode ?? prev.transport_mode,
    manual_places:
      command.params.manual_places && command.params.manual_places.length > 0
        ? Array.from(new Set([...prev.manual_places, ...command.params.manual_places]))
        : prev.manual_places,
  }));
};
  ...
  return (
  <div
     className="min-h-screen relative overflow-x-auto overflow-y-hidden"
...
opacity: 1.55,
    ...
    <div className="relative z-10">
      <div className="max-w-[1700px] min-w-[1700px] mx-auto px-6 py-5">
        {/* Header */}
